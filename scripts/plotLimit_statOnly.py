...
for i in signalList:
    ...
    f = TFile(filepath+"/run/"+indir+"/statonly/"+i+"/Limits/asymptotics/"+i+suffix_SR+"_statonly_BLIND_CL95.root")
...
for i in ['.eps', '.png', '.pdf']:
    clim.SaveAs(outpath+"massLimit_statonly_%s_%s%s"%(SR,suff,i))
...
fout = TFile(outpath+"/massLimit_statonly_%s_%s.root"%(SR,suff),"RECREATE")
xsec.Write("theory")
exp.Write("expected")
obs.Write("observed")
