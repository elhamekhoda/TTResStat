...
def plot_limits(run_dir):
    ...
    for limit_file in limit_files:
        ...
        ftxt.close()
        m_tev = m / 1000.
        print(m_tev, "TeV\tlimits in pb, exp: %10f (1 sigma: +%10f -%10f), (2 sigma: +%10f -%10f)" % (muexp *
              cross_section, muexp_p1*cross_section, muexp_m1*cross_section, muexp_p2*cross_section, muexp_m2*cross_section))
        sigma1.SetPoint(idx, m_tev, muexp*cross_section)
        sigma1.SetPointError(idx, 0, 0, muexp_m1 *
                             cross_section, muexp_p1*cross_section)
        sigma2.SetPoint(idx, m_tev, muexp*cross_section)
        sigma2.SetPointError(idx, 0, 0, muexp_m2 *
                             cross_section, muexp_p2*cross_section)
    ...
    fout = TFile(str(
        out_dir/(f"{settings['signal']}_{settings['channel']}_limit.root")), "RECREATE")
    xsec.Write("theory")
    exp.Write("expected")
    # obs.Write("observed")
